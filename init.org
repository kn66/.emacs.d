...
補完候補のフィルタリングとマッチングを強化するパッケージです。
...
#+begin_src emacs-lisp
  (use-package orderless
    ...
    (with-eval-after-load 'gnus-util
      (setopt gnus-completion-styles '(orderless basic))))
#+end_src
...
#+begin_src emacs-lisp
  (use-package vertico
    :init
    (setopt vertico-sort-function #'vertico-sort-history-alpha)
    (vertico-mode))
#+end_src
