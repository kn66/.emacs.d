...
2. Emacsを起動し、 ~M-x org-babel-tangle~ を実行
...
実際のメールサーバーの情報などは個人情報が含まれるため ~~/.authinfo.gpg~ や ~~/.gnus~ に記述しています。
...
#+begin_src emacs-lisp
  (use-package gnus
    ...
    :after bbdb
    :init
    (setopt gnus-select-method '(nnnil "") ; メインのニュースサーバー設定（使用しないのでnnnil）
            gnus-secondary-select-methods '((nnml "")) ; ローカルに保存
            send-mail-function 'smtpmail-send-it ; メール送信にsmtpmailを使用
            message-send-mail-function 'smtpmail-send-it ; メール送信にsmtpmailを使用
            gnus-message-archive-group "nnml:bogus" ; 送信メールの保存先指定
            )
    :config
    (gnus-demon-init))
#+end_src
...
*** [[https://github.com/trueroad/tr-emacs-ime-module][tr-ime]] (Windows向け)

Windows環境でOS標準のIME (MS-IMEなど) をEmacs内でスムーズに利用するための設定です。
...
LinuxやmacOS環境でMozc (Google日本語入力のオープンソース版) をEmacsの入力メソッドとして利用するための設定です。
[[https://www.kkaneko.jp/tools/server/mozc.html][日本語変換 Mozc の設定，emacs 用の Mozc の設定（Ubuntu 上）]]を参考にしています。

**** 必要なパッケージを導入 (Ubuntuの場合)

#+begin_src shell
  sudo apt install fcitx-libs-dev # Fcitxライブラリ (開発用)
  sudo apt install emacs-mozc     # Emacs用Mozcパッケージ
  fcitx-config-gtk                # Fcitx設定ツール (GUI)
#+end_src
...
gnus等でorderlessが効かないシチュエーションがあったため、絞り込みにもprescientを使用しています。
...
ここでは、TypeScriptおよびJSX/TSXファイル保存時にPrettierを自動実行する設定例を示します。
