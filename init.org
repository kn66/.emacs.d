...
#+begin_src emacs-lisp
  (use-package package
    ...
    (unless (version< emacs-version "31.0")
      ;; Emacs31から組み込みでアップデートのレビュー機能が導入されています
      (setopt package-review-policy t)))
#+end_src

*** [[https://github.com/kn66/package-git.el][package-git]]

elpaディレクトリ上にtarball形式のパッケージを管理する用のリポジトリを作成し、自動的に管理するための自作パッケージです。
...
パッケージ更新時に差分を表示するパッケージです。
Emacs31から標準機能で搭載されます。
...
Tab文字ではなくスペースを使ってインデントするための基本設定です。
モードごとに明示的な上書きがない限り、スペース主体の整形に揃えます。
...
~org-agenda~ は、複数のOrgファイルからTODOや予定を集約して確認するための標準機能です。
ここでは対象ディレクトリを ~/org/~ に設定しています。
...
[[https://github.com/emacs-evil/evil-collection/blob/master/modes/corfu/evil-collection-corfu.el][evil-collection]]の設定を元にキーバインドを設定しています。
補完候補未選択時にRETを押下すると改行が挿入され、補完候補選択時にRETを押下すると補完候補を選択します。
...
Verticoと標準補完機能が噛み合うように、ミニバッファ関連の基本挙動を調整します。
再帰的ミニバッファや大文字小文字を無視した補完などをここでまとめて設定しています。
...
複数候補をまとめて選ぶ ~completing-read-multiple~ のプロンプトを分かりやすくする補助設定です。
どの区切り文字で複数選択するかをミニバッファ上で確認しやすくします。
...
vertico-bufferをchild frameで表示するパッケージです。
プレビュー機能もついています。
...
*** 文字コードを指定してripgrepするコマンド

~ripgrep~ は検索対象ファイルの文字コードを ~--encoding~ で指定できます。
UTF-8以外のWindows由来のファイルを検索したい場合は、Shift-JIS/CP932を選択して検索します。
...
~eglot~ 周辺の補助パッケージをまとめたセクションです。
言語サーバーの導入や設定の共通化、スニペット連携を扱います。

*** [[https://github.com/kn66/eglot-multi-preset][eglot-multi-preset]]

[[https://github.com/joaotavora/rassumfrassum][rass (rassumfrassum)]] や [[https://github.com/thefrontside/lspx][lspx]] とeglotをスムーズに設定するパッケージです。
