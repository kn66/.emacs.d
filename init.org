...
カーソルがある行を背景色で強調し、現在位置を見失いにくくします。
特に分割ウィンドウや長いバッファでの移動時に有効です。
...
ef-themesを使用して独自テーマを作成・適用しています。

#+begin_src emacs-lisp
  ;; ~/.emacs.d/lisp/ef-*.el を load-theme の探索対象に追加
  (add-to-list 'custom-theme-load-path
               (expand-file-name "themes" user-emacs-directory))

  (use-package ef-themes
    :config
    (load-theme 'ef-hidamari t))
#+end_src
  ...
  (use-package corfu
    ...
    (with-eval-after-load 'orderless
      (add-hook 'corfu-mode-hook
                (lambda () (setq-local orderless-matching-styles '(orderless-flex)))))
  ...
  (use-package fontaine
    ...
    (cond (IS-LINUX
           (setopt fontaine-presets
                   '((regular
                      :default-family "Noto Sans Mono CJK JP"
...
#+begin_src emacs-lisp
  (use-package pulsar
    :config
    (setopt pulsar-face 'pulsar-yellow)
    (pulsar-global-mode))
#+end_src
...
こちらは ~indent-bars~ をより広いモードで一律有効にするための追加設定です。
前段の遅延有効化設定とは別に、汎用的なフック設定をまとめています。
...
#+begin_src emacs-lisp
  (use-package lsp-installer
    :load-path "~/.emacs.d/lisp/lsp-installer.el"
    :config
    (lsp-installer-setup))
#+end_src
