...
** [[https://github.com/kn66/fortune-gold-theme.el][fortune-gold-theme.el]]

自作テーマです。

#+begin_src emacs-lisp
  (use-package fortune-gold-theme
    :vc ( :url "https://github.com/kn66/fortune-gold-theme.el"
          :rev :newest)
    :config
    (load-theme 'fortune-gold t))
#+end_src

** [[https://github.com/kn66/folio-theme.el][folio-theme.el]]

自作テーマ2です。

#+begin_src emacs-lisp :tangle no
  (use-package folio-theme
    :load-path "~/.emacs.d/lisp/folio-theme.el/"
    :config
    ;; (load-theme 'folio t)
    )
#+end_src
...
Orgバッファの見た目を整え、見出し記号やチェックボックスなどをモダンな表示に置き換えるパッケージです。
編集体験はそのままに、閲覧時の視認性を高められます。
...
~org-modern~ と組み合わせて、Org文書のインデント表示をより自然に見せる補助パッケージです。
見た目の整合性を保ちながら階層構造を把握しやすくします。
...
#+begin_src emacs-lisp
  (use-package cape
    ...
    (add-hook 'prog-mode-hook #'my/set-super-capf)
    (add-hook 'text-mode-hook #'my/set-super-capf)
    (add-hook 'conf-mode-hook #'my/set-super-capf)
    (add-hook 'eglot-managed-mode-hook #'my/set-super-capf))
#+end_src
...
** [[https://github.com/kn66/nucleo-completion.el][nucleo-completion.el]]

nucleoライブラリを使用した補完パッケージ。
動的モジュールによる補完なのでパフォーマンスが良いです。

#+begin_src emacs-lisp
  (use-package nucleo-completion
    :vc ( :url "https://github.com/kn66/nucleo-completion.el.git"
          :rev :newest)
    ;; :load-path "~/.emacs.d/lisp/nucleo-completion.el/" ; 検証用
    :config
    (setopt completion-styles '(nucleo basic)
            completion-category-overrides '((eglot (styles nucleo basic))
    ...
    (with-eval-after-load 'migemo
      (defvar my-nucleo-completion-migemo-regexp-cache
        (make-hash-table :test #'equal))

      (defun my-nucleo-completion-clear-migemo-regexp-cache ()
        (clrhash my-nucleo-completion-migemo-regexp-cache))

      (defun my-nucleo-completion-migemo-regexp (term)
        (when (not (string-empty-p term))
          (let ((cached (gethash term
                                 my-nucleo-completion-migemo-regexp-cache
                                 :my-nucleo-cache-miss)))
            (if (not (eq cached :my-nucleo-cache-miss))
                cached
              (when (> (hash-table-count
                        my-nucleo-completion-migemo-regexp-cache)
                       1024)
                (my-nucleo-completion-clear-migemo-regexp-cache))
...
#+begin_src emacs-lisp
  (with-eval-after-load 'corfu
    (defun my-corfu-nucleo-history-sort (candidates)
      "Apply display-sort-function, then promote Corfu history without breaking Nucleo order."
      (let* ((display-sort-func (corfu--metadata-get 'display-sort-function))
             (candidates
              (if display-sort-func
                  (funcall display-sort-func candidates)
                candidates)))
        (if (and (boundp 'corfu-history)
                 corfu-history)
            (let ((rank (make-hash-table :test #'equal)))
              ...
              (cl-stable-sort
               candidates
               (lambda (a b)
                 (< (gethash (substring-no-properties a) rank most-positive-fixnum)
                    (gethash (substring-no-properties b) rank most-positive-fixnum)))))
          candidates)))
...
#+begin_src emacs-lisp
  (use-package indent-bars
    :hook ((python-base-mode-hook
            ...
            java-mode-hook
            java-ts-mode-hook
            csharp-ts-mode-hook) . indent-bars-mode))
#+end_src
